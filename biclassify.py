...
# Filter images that exist in a.txt
filtered_images = []
for line in label_txt_lines:
    image_name, label = line.strip().split(',')
    # Use regex to match leading digits
    ...
    if match:
        ...
        if image_number in a_txt_numbers:
            ...
            filtered_images.append([image_name, label_value])  # Note this is a list to match read_label_file output

# Convert filtered images and labels to DataFrame, ensure columns match read_label_file output
...
class ResNetClassifier(nn.Module):
    def __init__(self, num_classes=2):
        ...
        # Freeze all pretrained layers' parameters
