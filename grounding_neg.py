...
# Function to convert words to singular form
def to_singular(word):
    singular_word = p.singular_noun(word)
    return singular_word if singular_word else word

# Define ClipClassifier class
...
# Initialize and load binary classification model
...
# Check whether a patch is valid
...
def process_images(text_file_path, dataset_path, model, preprocess, binary_classifier, output_folder, device='cpu'):
    ...
    with open(text_file_path, 'r') as f:
        for line in f:
            ...
            boxes_dict[image_name] = [(patches_object[idx].cpu().numpy() * np.array([w, h, w, h])).tolist() for idx in top_3_indices]

    return boxes_dict

def main(args):
    # Set fixed default paths
    ...
    # Process images and generate bounding boxes
