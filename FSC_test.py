...
class TestData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load negative sample boxes
        neg_anno = self.neg_annotations[im_id]  # assume each image ID has a corresponding entry in the negative annotations
...
def batched_rmse(predictions, targets, batch_size=100):
    """
    Compute RMSE in batches.
    :param predictions: Model predictions, a PyTorch tensor
    :param targets: Ground truth values, a PyTorch tensor with the same shape as predictions
    ...
    for i in range(0, len(predictions), batch_size):
        ...
        # Ensure using float64 for higher precision
...
def batched_mae(predictions, targets, batch_size=100):
    ...
    for i in range(0, len(predictions), batch_size):
        ...
        # Compute absolute errors for the batch
        absolute_errors = torch.abs(batch_predictions - batch_targets)
        
        # Accumulate absolute errors and counts
